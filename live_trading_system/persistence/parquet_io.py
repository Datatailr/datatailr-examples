...
* in local development (no ``datatailr`` package available) we fall back
  to a filesystem-backed mock rooted at ``LIVE_TRADING_SYSTEM_LOCAL_BLOB_DIR``
  (default ``/tmp/live_trading_system_blob``).

The on-disk layout under either backend is::

    live_trading_system/trades/dt=YYYY-MM-DD/HHMMSS-<seq>.parquet
    live_trading_system/positions/latest.parquet
    live_trading_system/positions/history/dt=YYYY-MM-DD/HHMMSS-<seq>.parquet
...
log = logging.getLogger("live_trading_system.persistence")

DEFAULT_BLOB_ROOT = "live_trading_system"
...
def get_blob():
    ...
    if os.environ.get("LIVE_TRADING_SYSTEM_FORCE_LOCAL_BLOB", "").lower() in (
        "1", "true", "yes"
    ):
        root = os.environ.get(
            "LIVE_TRADING_SYSTEM_LOCAL_BLOB_DIR", "/tmp/live_trading_system_blob"
        )
...
def _join_with_overlap(prefix: str, name: str) -> str:
    """Join *prefix* + *name*, collapsing any path-segment overlap.

    The two blob backends disagree on what ``ls()`` puts in ``name``:

    * :class:`_LocalFsBlob.ls` returns the full path under the workspace
      root (e.g. ``live_trading_system/trades/dt=.../foo.parquet``).
    * :class:`datatailr.Blob.ls` returns the path **relative to the
      bucket root** (e.g. ``trades/dt=.../foo.parquet`` when the
      bucket is ``live_trading_system``) or just the basename when called
      with a non-recursive prefix.

    Without overlap detection a naive ``prefix + name`` produces paths
    like ``live_trading_system/trades/dt=.../trades/dt=.../foo.parquet``,
    which then 404 on ``blob.get_file``.

    This helper finds the longest trailing path-segment of *prefix*
    that is also a leading path-segment of *name* and emits a single,
    de-duplicated absolute path.
    """
    ...
    p_segments = norm_prefix.rstrip("/").split("/")
    for k in range(len(p_segments), 0, -1):
        trailing = "/".join(p_segments[-k:]) + "/"
        if name.startswith(trailing):
            head = "/".join(p_segments[:-k])
            return (head + "/" + name) if head else name

    return norm_prefix + name
...
def _normalise_ls_entries(entries: Iterable[Any], prefix: str) -> List[str]:
    ...
    * ``_LocalFsBlob.ls`` returns ``List[str]`` of paths already prefixed
      with the workspace root.
    * ``datatailr.Blob.ls`` returns ``List[dict]`` where ``name`` is the
      bucket-relative path (e.g. ``trades/dt=2026-05-28/foo.parquet``
      when listing ``live_trading_system/trades/dt=2026-05-28/``).
    * Older mocks return basename-only strings.

    The result is a flat list of full paths (no ``blob://`` scheme,
    no leading ``/``, no duplicated path segments).
    """
    out: List[str] = []
    for entry in entries:
        ...
        if not name:
            continue
        out.append(_join_with_overlap(prefix, name))
    return out
