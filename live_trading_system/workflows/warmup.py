...
from live_trading_system.workflows.zmq_client import broadcast, ctl_request

logger = logging.getLogger("live_trading_system.workflows.warmup")
...
BLOB_BUCKET = os.environ.get("LIVE_TRADING_SYSTEM_BLOB_BUCKET", "live_trading_system/eod")
...
def load_strategy_config() -> Dict[str, Any]:
    ...
    try:
        raw = _maybe_parse_json(kv.get("live_trading_system/strategies"))
        ...
        logger.info(
            "kv.get('live_trading_system/strategies') missing/invalid (%s); using defaults",
            exc,
        )

    risk_limits = dict(DEFAULT_RISK_LIMITS)
    try:
        raw = _maybe_parse_json(kv.get("live_trading_system/risk_limits"))
        ...
        logger.info(
            "kv.get('live_trading_system/risk_limits') missing/invalid (%s); using defaults",
            exc,
        )
