# Reactive Graph -- Specification

A reactive graph of long-running services and scheduled workflows
...
`Reactive Graph Dashboard` -- FastAPI single-page UI with:
...
* Deploying everything in `reactive_graph/deploy.py` plus the two
...
* Triggering `python workflows_deploy.py eod --local` produces:
  * a new blob under `reactive_graph/eod/<rundate>.json`,
