# Reactive Graph -- Trading Platform Demo
...
five-stage reactive graph, a FastAPI dashboard subscribes to every node
...
intent, and the `execution-simulator` fills approved orders. A new
...
```
   ...
   notification-bus  <----CTL broadcast----  workflows  <---------+
        ^                                       |
        |                                       v
        +--system events-->  Reactive Graph Dashboard  <---all nodes
...
| **Reactive Graph Dashboard** | App (FastAPI) | 8080 | Topology, ticker, analytics, live positions/PnL, blotter, system events; "Persisted history" panel runs DuckDB queries against the Parquet store on `/api/history` |
...
```bash
python reactive_graph/deploy.py             # all 7 services + dashboard
...
```bash
python reactive_graph/workflows_deploy.py             # both workflows
python reactive_graph/workflows_deploy.py warmup      # Pre-Market Warmup
python reactive_graph/workflows_deploy.py eod         # EOD Reconciliation
python reactive_graph/workflows_deploy.py warmup --local  # run locally
python reactive_graph/workflows_deploy.py eod --local
```
...
|-----|------|---------------|
| `reactive_graph/strategies` | JSON list | `["momentum", "mean_reversion"]` |
| `reactive_graph/risk_limits` | JSON object | `{"max_position": 1000, "max_notional": 250000, "max_daily_loss": 50000}` |
...
[`reactive_graph/persistence/parquet_io.py`](persistence/parquet_io.py),
...
duckdb -c "SELECT COUNT(*), SUM(qty*price) AS notional \
           FROM read_parquet('reactive_graph/trades/dt=2026-05-28/*.parquet')"

# current book
duckdb -c "SELECT * FROM read_parquet('reactive_graph/positions/latest.parquet')"
```
...
6. **Trigger the EOD workflow** (`python workflows_deploy.py eod
   ...
   `reactive_graph/eod/<rundate>.json` and broadcasts `eod_complete`.
   The dashboard's ribbon shows the event within seconds.
...
PYTHONPATH=. NODE_NAME=notification-bus NODE_ROLE=notification-bus \
  PORT=8085 python -m reactive_graph.node.app

# persistence-sink (writes Parquet to REACTIVE_GRAPH_LOCAL_BLOB_DIR
# when datatailr.Blob isn't available; defaults to /tmp/reactive_graph_blob)
...
# is on localhost.
PYTHONPATH=. REACTIVE_GRAPH_NODES=\
...
persistence-sink@localhost:8086 \
  PORT=8000 python -m reactive_graph.dashboard.app
```

`REACTIVE_GRAPH_NODES` accepts both plain `host[:port]` (Datatailr,
...
| Blob storage | EOD report at `reactive_graph/eod/<rundate>.json` |
| KV | `reactive_graph/strategies`, `reactive_graph/risk_limits` |
